...
async function convertBufferToStickerWebp(inputBuffer, isAnimated, cropSquare) {
  ...
  try {
    ...
    // Run ffmpeg with safety timeout to avoid hanging child processes
    await execWithTimeout(ffmpegCommand, { timeout: 30 * 1000 });
    ...
    try {
      ...
      // Use safe exec with timeout
      await execWithTimeout(harsherCmd, { timeout: 20000 });
    ...
    try {
      ...
      for (const size of sizes) {
        ...
        await execWithTimeout(cmdSmall, { timeout: 20000 });
