...
async function startBot() {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            console.log('PrimeSA_Bot is ready to receive messages!\n');
    ...
    sock.ev.on('messages.upsert', ({ messages, type }) => {
        ...
        for (const msg of messages) {
            ...
            // System message filter - ignore broadcast/newsletter messages but allow status@broadcast
            // so autostatus handler can process status updates
            if (isSystemJid(from) && from !== 'status@broadcast') {
                continue; // Silently ignore non-status system messages
            }
...
// Start the bot
console.log('🚀 Starting PrimeSA_Bot...\n');
console.log(`📦 Bot Name: ${config.botName}`);
console.log(`⚡ Prefix: ${config.prefix}`);
const ownerNames = Array.isArray(config.ownerName) ? config.ownerName.join(',') : config.ownerName;
console.log(`👑 Owner: ${ownerNames}\n`);
